<template>
    <div class="bc-fff">
        ...
        <div>
            ...
            <div class="products_reviews_content p-10" style="margin-top:46px;">
                <!-- 商品评论的类型选择 -->
                <div class="products_reviews_type">
                    <productReviewsType v-for="(items,index) in productReviewsTypeData" :num="count" :index="index" :typeStr="items.typeName" :type="items.type" v-on:type-product="activeProductType(index,items)"></productReviewsType>
                </div>
                <!-- 商品评论右侧弹出:评论列表 -->
                ...
                <div class="" v-show="isList" style="overflow:hidden" infinite-scroll-disabled="loading" infinite-scroll-distance="100" v-infinite-scroll="loadMore" infinite-scroll-immediate-check="ture">
                    <productsReviewsList v-for="(items,index) in productsReviewsList" :nickname="items.member_name" :headerImg="items.user_logo" :details="items.content" :imgArr="items.image_row" :photoIndex="index" :data-type="items.type">
                    </productsReviewsList>
                </div>
...
export default {
    ...
    methods: {
        ...
        getGoodsEvaluationList() {
            ...
            var _self = this;
            _self.common.get(
                _self.apiUrl.shopUrl+"/index.php?ctl=Goods_Goods&met=getGoodsEvaluationList&typ=json&sou=wap",
                {
